...
Some simple examples will help. We will first plot one of R's built-in
datasets, the [*Iris* flower
dataset](https://en.wikipedia.org/wiki/Iris_flower_data_set), which
measured sizes of iris flowers in different species, in the 1930s.

This data set records the length and width of two different parts of
flowers, the Sepal and the Petal, for three different species, (*Iris
...
visualise on a log scale, where equal spacings on the page represent
equal change in log space.

Here we use the built-in `scale_y_log10` to display the y-axis on a
log(10) scale.
...
This offers an alternative to splittting different factors by,
...
their appearance. Text elements vary in font. Most elements vary by
their size and color, and even by their presence or absence. ggplot
controls the appearance of these other plot elements with **themes**.

For example, plots with white background look more readable when
printed. We can set the background to white using built-in theme,
`theme_bw()`:
...
column. However, bar plots are commonly used to count the number of
observations in groups. ggplot distinguishes between `geom_col`, which
plots the raw dat supplied, and `geom_bar`, which does the counting for
you.

We can create barplots of counts using `geom_bar`: Let's make a barplot
showing the number of variants for each sample that are indels.
...
> facets, we don't need the legend. Use the help file for `geom_bar` and
> any other online resources you want to use to remove the legend from
> the plot.
>
> > ## Solution
> >
...
right plotting window in RStudio. Or you can write this into a script
using the function `ggsave` - remember, use `?ggsave` to ask for help
from your R session.
